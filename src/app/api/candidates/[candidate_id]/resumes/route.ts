...
export async function GET(
  ...
  try {
    ...
    const backendUrl = `${BACKEND_URL}/candidates/${candidate_id}/resumes`;

    console.log(`[API Proxy] GET /api/candidates/${candidate_id}/resumes -> ${backendUrl}`);
    ...
    console.log(`[API Proxy] Success: ${data.resumes?.length || 0} resumes returned`);

    return NextResponse.json(data);
  } catch (error) {
    console.error("[API Proxy] Error proxying to backend:", error);
    return NextResponse.json(
      { error: "Failed to fetch candidate resumes from backend" },
      { status: 500 }
    );
  }
}