...
export async function GET(
  ...
  try {
    ...
    const backendUrl = `${BACKEND_URL}/candidates/${candidate_id}/resume-health`;

    console.log(`[API Proxy] GET /api/candidates/${candidate_id}/resume-health -> ${backendUrl}`);
    ...
    console.log(`[API Proxy] Success: Resume health returned`);

    return NextResponse.json(data);
  } catch (error) {
    console.error("[API Proxy] Error proxying to backend:", error);
    return NextResponse.json(
      { error: "Failed to fetch resume health from backend" },
      { status: 500 }
    );
  }
}