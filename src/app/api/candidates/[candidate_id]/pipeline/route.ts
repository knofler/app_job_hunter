...
export async function GET(
  ...
  try {
    ...
    const backendUrl = `${BACKEND_URL}/candidates/${candidate_id}/pipeline`;

    console.log(`[API Proxy] GET /api/candidates/${candidate_id}/pipeline -> ${backendUrl}`);
    ...
    console.log(`[API Proxy] Success: Pipeline returned`);

    return NextResponse.json(data);
  } catch (error) {
    console.error("[API Proxy] Error proxying to backend:", error);
    return NextResponse.json(
      { error: "Failed to fetch pipeline from backend" },
      { status: 500 }
    );
  }
}