  ...
  return (
    <div className="max-w-5xl mx-auto py-10 px-4 flex flex-col md:flex-row gap-8">
      <div className="w-full md:w-1/2">
        ...
        {usingFallback && (
          <div className="text-xs text-gray-400 mb-2">Showing cached applications while the API is unavailable.</div>
        )}
        ...
        <ul className="flex flex-col gap-2">
          {applications.map(application => {
            ...
            const statusStyle = STATUS_STYLES[status] ?? "bg-gray-100 text-gray-700 border-gray-300";
            ...
            return (
              <li
                key={application.id}
                className={`bg-white rounded-lg px-4 py-3 flex flex-col items-start shadow transition-all duration-200 cursor-pointer ${selectedId === application.id ? "ring-2 ring-blue-400" : ""}`}
                style={{ minHeight: 48 }}
                onClick={() => setSelectedId(application.id)}
              >
                <div className="flex items-center gap-2 w-full">
                  <span className="font-semibold flex-1 text-gray-900">{job.title ?? "Untitled Role"}</span>
                ...
                {resumeLabel && (
                  <div className="text-xs text-gray-500">Resume: <span className="font-semibold text-blue-700">{resumeLabel}</span></div>
                )}
      ...
      <div className="w-full md:w-1/2">
        <div className="bg-white border-2 border-blue-200 rounded-lg p-6 shadow-sm min-h-[300px]">
          {selectedApplication ? (
            ...
            <div className="text-sm text-gray-500">Select an application to preview its details.</div>
          )}
        </div>
      </div>
    </div>
