...
export default function ResumeHealthCard() {
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-6 flex flex-col items-center">
      {!candidateId ? (
        <div className="text-xs text-muted-foreground">Switch to the candidate persona to view resume health.</div>
      ) : loading ? (
        <div className="text-muted-foreground text-sm">Loading...</div>
      ) : (
        <>
          <div className="text-3xl font-bold text-blue-600">{score}/100</div>
          <div className="text-sm text-muted-foreground mb-4">Resume Health</div>
          ...
          {usingFallback && (
            <div className="text-xs text-muted-foreground mt-3">Showing cached data while the API is unavailable.</div>
          )}
        </>
      )}
    </div>
